...
- **Four-tab interface** — floating "morph" capsule tabs (the active one
  expands into an accent pill with its label) split the app into Control (dial,
  modes, zones, schedules), Activity, Trends (runtime and temperature graphs),
  and System (controller health and filter maintenance), with hash routing so
  the phone's back button moves between tabs and scroll positions are preserved.
